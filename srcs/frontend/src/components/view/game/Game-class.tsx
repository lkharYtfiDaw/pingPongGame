...
//=========================Events==============================================
    ...
    setGameEvents()
    {
        ...
        this.socket.on("Auth failed",()=>{window.location.href = window.location.origin});//should get redirected to login page
        if (this.role == "player")
        {
        ...
        this.socket.on("game has ended", ()=>
        {
            clearInterval(this.conIntervalId);
            this.count = 4;
            this.renderEnd("Game Ended");
        })
        });
...
//=========================Drawing in Canvas====================================
    ...
    renderEnd(result:string)
    {
        ...
        else if (result == "Right Player Won" || result == "Left Player Won")
            this.text(result,this.context.canvas.width * .4, this.context.canvas.height* .7, "WHITE", `${this.context.canvas.width / 24}px system-ui`);
        else
